...
/----- (0000000000500704) ----------------------------------------------------
void __fastcall YS::trap_obj_look_start(BD_VALUE_21 *args)
{
  __int64 v2; // r29
  unsigned __int64 v3; // r28
  __int64 v4; // r30
  YS::LOOKAT_116 *v5; // r29
  ...
  v5 = (YS::LOOKAT_116 *)(unsigned int)(*(_DWORD *)(v2 + 4) + 1184);
  if ( !(_DWORD)v4 )
  {
    ErrorPrintf(
      "#ASSERT(%s):%s/%s:%d:%s\nret=%p\n",
  ...
  YS::LOOKAT::start(v5, (YS::OBJ_117 *)*(unsigned int *)(v4 + 4), *(float *)&(*args)[8], 1u);
}
// 500728: variable 'back_chain' is possibly undefined
->
/----- (00000000004451F8) ----------------------------------------------------
void __fastcall YS::LOOKAT::start(YS::LOOKAT_116 *const this, const kn::FVector *pos, double time, u_int flag)
{
